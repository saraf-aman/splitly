...
- **Creator**: everything an Admin can do, plus can demote/remove other admins, plus can delete the entire household (wipes all members/bills/data, requires typing the household name to confirm). Cannot themselves ever be demoted or removed via any in-UI member-management action, and that path is enforced in Firestore rules, not just UI. The only two ways ownership changes hands: (a) as the last step of that full household deletion, or (b) automatically and silently when the creator deletes their own account (Phase 15) and another member exists to take over — see `PROJECT_PLAN.md` §17 for the transfer-eligibility rule. That server-side transfer runs via `firebase-admin`, which bypasses Firestore rules by design; the rules-level lock only ever applied to client writes.
