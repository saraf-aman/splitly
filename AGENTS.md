# AGENTS.md — Household Bill Splitter

This file is read automatically at the start of every Codex session. Keep it lean — it's a router, not the full spec. Detailed decisions live in `docs/PROJECT_PLAN.md`; the step-by-step build order lives in `docs/ROADMAP.md`; what's already built lives in `docs/PROGRESS.md`.
...
## How to work in this repo (for Codex, every session)
...
5. Before marking the step done, verify it live — don't rely on `tsc`/`lint`/`next build` passing as proof a UI step actually works:
   - If the step touches any UI, use the `Codex-in-chrome` tools against the user's already-running `npm run dev` instance (check for one first — don't spin up a second one) and screenshot/click through the actual change.
