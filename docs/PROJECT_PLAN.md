...
- **`NavDrawer` changes**: added a "Profile" row above "Home" (uses the user's Google avatar as its icon, falls back to `CircleUserRound`, separated from "Home" by a divider). Removed the personal Splitwise connect/disconnect block and the Sign out button entirely (both now live only on `/profile`). The household-level Splitwise **group link** (creator-only — linking *this household* to a Splitwise group) stays in `NavDrawer`, since that's genuinely household-scoped; the section is always shown (not gated on connection/role) so a disconnected member — creator or not — always sees a pointer to `/profile` ("Link your Splitwise account from Profile" / "...to connect this group" for creators) instead of the section just vanishing.
- **Delete account entry point**: deliberately a plain text button, same visual weight as "Sign out" — *not* a permanently-visible "Danger zone" card. Clicking it expands an inline confirm (same collapse/expand pattern as `NavDrawer`'s "Leave Group"), which requires typing the user's own email (shown quoted: `Type "x@y.com" to confirm`) to enable the destructive button. Lighter-weight than household deletion's typed-name confirmation was judged appropriate since this only destroys the acting user's own data. **Household deletion's UI was retrofitted to match this same pattern** (see below) rather than staying the odd one out.
...
User feedback after 15.3 shipped, all applied:

- **Household deletion UI harmonized with account deletion**: `src/app/groups/[groupId]/group/page.tsx`'s "Danger zone" card (permanently visible, casual copy) was replaced with the same pattern as `/profile`'s delete-account — a plain "Delete group" button that expands into an inline confirm on click. Copy tightened: *"Deleting {name} will permanently remove all of its members, bills, and items. This action is irreversible."*
- **Confirm-text inputs now show the exact string in quotes** on both delete flows — `Type "x@y.com" to confirm` / `Type "Household Name" to confirm` — previously unquoted and easy to misread as a description rather than a literal value.
- **Fixed an invisible-input bug** on both confirm panels: the shared `Input` (and `Button` outline variant, `Select` trigger, `Badge`) still shipped `dark:*` classes from the original shadcn template, which fire on any browser with system dark mode on. Since `globals.css` documents "no dark mode by deliberate decision," those classes were dead weight that only ever produced this one bug (background dropping to ~30% opacity, unreadable against a tinted panel) — stripped from all four `src/components/ui/*.tsx` files plus one stray instance in `NavDrawer.tsx`. Confirmed safe: nothing in the app intentionally depends on `prefers-color-scheme`.
- **Fixed `/profile`'s "Back" and the top-bar Splitly logo always going to the group picker**, regardless of where the user actually came from. "Back" now uses `router.back()`. The logo (which has no `hhId` to work from while on `/profile`) reads a small new `src/lib/profileReturn.ts` (`sessionStorage`, set by whichever drawer — `NavDrawer` or `PickerNavDrawer` — opened `/profile`) via `useSyncExternalStore` in `AppShell` (not `useState`+effect, to avoid the same `react-hooks/set-state-in-effect` issue noted elsewhere in this doc's history).
- **Restored Splitwise discoverability for non-connected, non-creator members**: moving the personal connect toggle to `/profile` had left `NavDrawer`'s Splitwise section fully hidden for anyone who wasn't a creator — no indication Splitwise existed at all. Now always shown when not connected, with role-aware copy: creators get "Link your Splitwise account from Profile to connect this group," everyone else gets a plain "Link your Splitwise account from Profile."
