...
**Phase 15 (account deletion) is done**, 15.1–15.3 all complete, plus a post-launch polish pass (see below). Phase 13 (backlog: email digest, smart defaults, per-bill notes, re-editable review) remains deferred as the next thing to consider.

**Most recent session:** Built out Phase 15 end to end, then did a polish pass off user feedback after it shipped. New account-level `/profile` page (avatar/name/email header, Splitwise connect/disconnect card showing the connected account's email, Sign out, Delete account) reached via a new "Profile" row in `NavDrawer` (above "Home", separated by a divider, user's Google avatar as its icon) and a new minimal `PickerNavDrawer` (just "Profile") giving the group-picker page the same hamburger-menu header as the rest of the app. Personal Splitwise connect/disconnect and Sign out moved out of `NavDrawer` entirely onto `/profile`; `NavDrawer` now only keeps the household-scoped Splitwise **group link** (creator-only), **always shown** when not connected (not just for creators, after feedback that non-creators lost all Splitwise discoverability) with role-aware copy. Splitwise callback (`/api/splitwise/callback`) now stores the connected account's `email`; a self-heal on `/profile` (`POST /api/splitwise/backfill-email`) backfills it for connections made before this change. Delete-account entry point is a plain button inside a bordered card with Sign out that expands an inline typed-email confirm on click — **household deletion's "Danger zone" card was later retrofitted to match this same collapse pattern**, and both confirm inputs now show the exact string to type in quotes.
...
**Polish pass (post-15.3, user-reported):** (1) Fixed a real bug, not just cosmetic — the confirm-panel `Input` (and `Button` outline variant, `Select` trigger, `Badge`) still carried leftover `dark:*` classes from the shadcn template; on a browser with system dark mode on, those made the confirm field render at ~30% opacity, nearly invisible against the tinted panel. Since `globals.css` documents this app as deliberately light-only, the classes were pure dead weight — stripped from all four `src/components/ui/*.tsx` files. (2) Fixed `/profile`'s "Back" and the top-bar logo always landing on the group picker regardless of origin — "Back" now uses `router.back()`; the logo reads a new `src/lib/profileReturn.ts` (`sessionStorage`, set by whichever drawer opened `/profile`) via `useSyncExternalStore` in `AppShell.tsx` to avoid a `set-state-in-effect` lint error.
