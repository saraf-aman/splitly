...
- [ ] **9.4** Bill history: settled bills older than 1 month are excluded from the default feed query (no deletion — they still exist in Firestore, just hidden). Non-settled bills (needs input / in progress) always appear regardless of age. No "show older" toggle needed.
