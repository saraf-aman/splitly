...
- [ ] **12.7** Smart defaults: auto-uncheck items a given user has consistently opted out of historically.
- [ ] **12.8** Per-bill notes field (e.g. "I'm paying for the wine separately, don't include me").
- [ ] **12.9** General mobile polish pass, offline/error state handling, loading states.
- [x] **12.10** Group retention policy, final design (deviates from original wording below per user direction): lives only in the Manage group page (`/groups/[groupId]/group`), creator-only (not admin-changeable, not on the group creation form). Options: 1, 3, 6, 12 months, or Never; default Never for new/never-configured groups. Stored as `retentionMonths: number | null` on the household doc (`null` = never hide). Client-side feed filter uses this value (replaced the hardcoded 30-day filter in `groups/[groupId]/page.tsx`) — settled bills older than the retention threshold are hidden from the feed immediately, regardless of whether the cron (12.11) has deleted them yet. Existing pre-12.10 groups were migrated to `retentionMonths: 6` so their hide behavior didn't silently change from ~30 days to never.
- [x] **12.11** Vercel cron — server-side bill deletion: `/api/cron/cleanup-bills` route, authenticated via Vercel's `CRON_SECRET`. Runs daily. Uses Firebase Admin SDK to fetch all groups with a non-null `retentionMonths`, queries settled bills older than their threshold, and batch-deletes each bill doc + its subcollections (items, sharedCharges — selections live inline on item docs, not a separate subcollection). Groups with `retentionMonths: null` ("Never") are skipped. Double-tier with 12.10: the client-side hide is the first layer (instant, free), the cron delete is the second layer (async cleanup). If the cron hits Firestore's free-tier daily write/delete quota, it stops and retries the next day — users never see the stale bills either way because the hide layer already filtered them.
