...
- [ ] **11.8** Splitwise button alignment on grid page: the "Push to Splitwise" button is currently positioned via a `ResizeObserver` spacer that measures the first member column width and adds a matching left margin — this is fragile and misaligns when the number of members changes or on certain viewport widths. Replace with a simpler, stable layout: align the button under the first data column using CSS (e.g. match the sticky item-name column width with a fixed min-width or use a proper table-footer row) so it stays correctly placed without JavaScript measurement.
- [ ] **11.9** Group retention policy — set on creation + admin-changeable: add a "Bill retention" field to the group creation form (options: 1, 3, 6, 12 months, or Forever; default: Forever). Stored as `retentionMonths: number | null` on the household doc (null = forever). Admins can change it later in group settings. Client-side feed filter uses this value (replaces the current hardcoded 30-day filter) — settled bills older than the retention threshold are hidden from the feed immediately, regardless of whether the cron has deleted them yet.
- [ ] **11.10** Vercel cron — server-side bill deletion: `/api/cron/cleanup-bills` route, authenticated via Vercel's `CRON_SECRET`. Runs daily. Uses Firebase Admin SDK to fetch all groups with a non-null `retentionMonths`, queries settled bills older than their threshold, and batch-deletes each bill doc + its subcollections (items, selections). Groups with `retentionMonths: null` are skipped. Double-tier with 11.9: the client-side hide is the first layer (instant, free), the cron delete is the second layer (async cleanup). If the cron hits Firestore's free-tier daily delete limit, it stops and retries the next day — users never see the stale bills either way because the hide layer already filtered them.
