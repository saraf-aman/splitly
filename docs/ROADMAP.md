...
- [ ] **12.6** Manual "remind" action: add a "Remind everyone" button inside the confirmed-members banner (same banner as the grid/select screens), visible to the bill uploader only. Tapping pushes a reminder to all not-yet-confirmed participants immediately, rate-limited to once per 24h per bill via `bills/{billId}.lastManualReminderAt` — button is disabled with a "You can remind again in Xh" state until the cooldown clears.
