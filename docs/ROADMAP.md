...
- [ ] **12.5** Automated reminder nudges: scheduled Vercel cron (`/api/cron/remind-bills`, `CRON_SECRET`-authenticated, e.g. hourly) scans `open` bills and pushes a reminder to any participant (excluding the uploader) who hasn't hit "confirm my selections" yet. Cadence per bill per member: first reminder 24h after the bill opened, then every 48h after that, capped at 3 total reminders — after the cap, that member goes silent until they act or the uploader manually nudges (12.6). Track state on the bill doc: `reminders.{uid}: { count: number, lastSentAt: timestamp }`.
