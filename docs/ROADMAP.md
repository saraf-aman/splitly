...
- [ ] **12.4** "Everyone's done" notification: when the last participant (per `participantIds` from 12.1, uploader included if they're a participant) flips their Phase 5.4 "confirm my selections" indicator to done, trigger a push to the uploader only: *"Everyone's made their picks on [bill name] — check the final split."* Reuses the existing FCM send path from Phase 7.2.
