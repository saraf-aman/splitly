...
- [x] **12.2** Bill participant scoping — add/remove after creation: uploader-only (not admin) control, reachable from the bill's review/select/grid screens. Adding someone appends their uid to `participantIds` and they immediately see the bill. Removing someone is only allowed if that member has made zero selection changes on the bill (still at all-default `included:true, shares:1` for every item) — if they've touched anything, block the removal with a dialog: *"[Name] has already made selections on this bill. Ask them to either reset their picks, or clear their selections yourself."* (the uploader can already edit any member's selections via the Phase 6.4 grid override). Adds and removes each trigger a push to the affected member (added → "tap to select your items"; removed → notified they no longer have access).
